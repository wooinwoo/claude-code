...
function fileStatusLetter(status) {
  ...
  if (s === 'A') return 'A'; if (s === 'D') return 'D'; if (s === 'R') return 'R';
  return 'M';
}
...
function renderDiffSidebar(stagedFiles, unstagedFiles) {
  ...
  const buildGroup = (label, files, section) => {
    ...
    const discardIcon = !isStaged ? `<button class="ds-action" onclick="event.stopPropagation();diffDiscardAll()" title="Discard All"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button>` : '';
    ...
    for (const f of files) {
      ...
      const discardBtn = !isStaged ? `<button class="fi-action fa-discard" onclick="event.stopPropagation();diffDiscardFile('${efn}')" title="Discard"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 4v6h6"/><path d="M3.51 15a9 9 0 105.64-8.36L1 10"/></svg></button>` : '';
  ...
  sb.querySelectorAll('.ds-group-head').forEach(h => {
    h.addEventListener('click', e => { if (e.target.closest('.ds-action')) return; h.parentElement.classList.toggle('collapsed'); });
  });
...
export async function updateDiffBranchInfo() {
  ...
  try {
    ...
    if (data.worktrees?.length > 1) html += `<span class="dbi-wt" onclick="toggleWorktreeDropdown(event)">${data.worktrees.length} worktrees</span>`;
...
export async function toggleBranchDropdown(e) {
  ...
  try {
    ...
    if (remotes.length) {
      ...
      for (const b of remotes) { const short = b.replace(/^origin\//, ''); html += `<div class="bd-item" data-branch="${esc(short)}" onclick="switchBranch('${esc(short).replace(/'/g, "\\'")}')"><span class="bd-check"></span><span class="bd-name">${esc(b)}</span></div>`; }
      html += '</div>';
    }
  ...
  } catch (err) { dd.innerHTML = `<div style="padding:12px;text-align:center;color:var(--red);font-size:.76rem">Error: ${err.message}</div>`; }
...
function renderAutoCommitPlan() {
  ...
  overlay.innerHTML = `<div class="ac-header"><h3><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v18M5.5 8.5l13 7M18.5 8.5l-13 7"/></svg>Auto Commit Plan</h3>${branchHtml}<div class="ac-stats"><span class="acs-commits">${plan.commits.length} commit${plan.commits.length > 1 ? 's' : ''}</span><span class="acs-files">${totalFiles} file${totalFiles > 1 ? 's' : ''}</span></div><button class="ac-cancel" onclick="cancelAutoCommit()">Cancel</button></div><div class="ac-body" id="ac-body"></div><div class="ac-footer" id="ac-footer"><div class="ac-progress" id="ac-progress" style="display:none"><div class="ac-progress-bar"><div class="ac-progress-fill" id="ac-progress-fill"></div></div><span class="ac-progress-text" id="ac-progress-text">0/${plan.commits.length}</span></div><button class="ac-commit-btn" id="ac-commit-btn" onclick="executeAutoCommit()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>Commit All (${plan.commits.length})</button></div>`;
...
export async function executeAutoCommit() {
  ...
  if (!failed && completed === activeCommits.length) {
    footer.innerHTML = `<div class="ac-done-msg"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M9 12l2 2 4-4"/><circle cx="12" cy="12" r="10"/></svg>${completed} commit${completed > 1 ? 's' : ''} done!</div><div style="flex:1"></div><button class="ac-cancel" onclick="cancelAutoCommit();loadDiff();">Close</button><button class="ac-push-btn" onclick="doPush('${projectId}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 19V5M5 12l7-7 7 7"/></svg>Push</button>`;
