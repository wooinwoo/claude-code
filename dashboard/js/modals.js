...
function renderSettingsProjectList() {
  document.getElementById('settings-project-list').innerHTML = app.projectList
    .map(p => `
    <div class="spi">
...
export async function browseTo(dir) {
  ...
  try {
    ...
    list.innerHTML = data.entries.map(e =>
      `<li class="fp-item" onclick="browseTo('${e.path.replace(/'/g, "\\'")}')">\
...
export function populateProjectSelects() {
  const opts = app.projectList.map(p => `<option value="${p.id}">${esc(p.name)}</option>`).join('');
...
export function updateDevBadge() {
  ...
  if (badge) badge.style.display = app.devServerState.length ? '' : '';
}
...
function buildCommandList() {
  ...
  cmds.push({ group: 'Navigation', label: 'Go to README', hint: 'Ctrl+4', icon: navIcon, action: () => window.switchView?.('readme') });
...
function renderCommandList() {
  ...
  app._cmdFiltered.forEach((cmd, i) => {
    ...
    html += `<div class="cmd-palette-item${i === app._cmdActiveIdx ? ' active' : ''}" data-idx="${i}" onmouseenter="setCmdActive(${i})" onclick="execCmd(${i})">${cmd.icon}<span class="cpi-label">${cmd.label}</span>${cmd.hint ? `<span class="cpi-hint">${cmd.hint}</span>` : ''}</div>`;
  });
  list.innerHTML = html;
...
function ensurePreviewModal() {
  ...
  el.onclick = e => { if (e.target === el) closeFilePreview(); };
...
export function openFilePreview(filePath) {
  ...
  fetch('/api/file?path=' + encodeURIComponent(filePath))
    .then(r => r.json())
    .then(data => {
      if (data.error) { body.innerHTML = `<div style="padding:20px;color:var(--red)">${data.error}</div>`; return; }
...
export function copyFileContent() {
  const el = document.getElementById('file-preview-overlay');
  if (el?._content) navigator.clipboard.writeText(el._content).then(() => showToast('Content copied')).catch(() => {});
}
