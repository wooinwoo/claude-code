...
export function connectWS() {
  ...
  app.ws.onclose = () => {
    ...
    app._wsReconnTimer = setTimeout(connectWS, app._wsBackoff + Math.random() * 500);
  };
}
...
export function addTerminal(termId, projectId, addToView) {
  ...
  const xterm = new Terminal({
    theme: { background: '#08090d', foreground: '#e8e9f0', cursor: '#818cf8', selectionBackground: 'rgba(129,140,248,.3)', black: '#08090d', red: '#f87171', green: '#34d399', yellow: '#fbbf24', blue: '#60a5fa', magenta: '#c084fc', cyan: '#22d3ee', white: '#e8e9f0' },
    fontFamily: "'Cascadia Code','JetBrains Mono',monospace",
    fontSize: app.termFontSize,
    cursorBlink: true, allowProposedApi: true, scrollback: 5000, fastScrollModifier: 'alt', fastScrollSensitivity: 5,
  });
  ...
  xterm.attachCustomKeyEventHandler(ev => {
    ...
    if ((ev.ctrlKey && ev.shiftKey && ev.code === 'KeyC') || (ev.ctrlKey && !ev.shiftKey && ev.key === 'c' && xterm.hasSelection())) {
      ...
      if (text) { const ta = document.createElement('textarea'); ta.value = text; ta.style.cssText = 'position:fixed;left:-9999px'; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta); xterm.clearSelection(); }
      return false;
    }
  ...
  xterm.onData(data => {
    if (app.ws?.readyState === 1) app.ws.send(JSON.stringify({ type: 'input', termId, data }));
  });
...
export function updateTermHeaders() {
  document.querySelectorAll('.split-leaf').forEach(leaf => {
    ...
    const cacheKey = `${t.label}|${t.color}|${g.branch || ''}|${g.uncommittedCount || 0}|${model}|${wt.length}|${tid === app.activeTermId}|${bufPct}`;
    ...
    head.innerHTML = `<span class="th-dot" style="background:${t.color}"></span>` +
      ...
      (g.branch ? `<span class="th-tag th-branch">${g.branch}</span>` : '') + wtTag +
      ...
      (bufPct >= 80 ? `<span class="th-tag" style="color:${bufPct >= 95 ? 'var(--red)' : 'var(--yellow)'};font-size:.7rem" title="Buffer: ${bufPct}% (${bufUsed}/5000 lines)">${bufPct}%</span>` : '') +
    ...
    head.ondblclick = e => { if (e.target.dataset.action === 'close') return; e.stopPropagation(); startRenameHeader(tid, head); };
  });
}
...
export function showTermCtxMenu(e, termId) {
  ...
  menu.onclick = ev => {
    ...
    switch (item.dataset.act) {
      ...
      case 'paste': navigator.clipboard.readText().then(txt => { if (txt && app.ws?.readyState === 1) app.ws.send(JSON.stringify({ type: 'input', termId, data: txt })); }).catch(() => {}); break;
...
function openNewTermModalWithSplit(targetTermId, pos) {
  ...
  sel.innerHTML = app.projectList.map(p => `<option value="${p.id}">${p.name}</option>`).join('');
...
export function doTermSearch(dir) {
  ...
  let found;
  if (dir === 'next') found = t.searchAddon.findNext(q, { regex: false, caseSensitive: false, incremental: true });
  else found = t.searchAddon.findPrevious(q, { regex: false, caseSensitive: false, incremental: true });
...
export function initFileDrop() {
  ...
  document.addEventListener('dragenter', e => { if (e.dataTransfer?.types?.includes('Files')) { e.preventDefault(); overlay.style.display = 'flex'; } });
  ...
  document.addEventListener('drop', e => {
    ...
    e.preventDefault();
    window.openFilePreviewFromFile?.(files[0]);
  });
}
  ...
  try {
    ...
    if (app._branchData.worktrees?.length > 1) {
      ...
      for (const wt of app._branchData.worktrees) {
        ...
        html += `<div class="nt-branch-item" data-type="worktree" data-value="${esc(wt.branch)}" data-cwd="${esc(wt.path)}" onclick="selectBranch(this)"><span>${esc(wt.branch)}</span><span class="nb-wt-path">${esc(name)}</span>${isCurrent ? '<span class="nb-current">current</span>' : ''}</div>`;
      }
      html += '</div>';
    }
    if (app._branchData.local?.length) {
      html += `<div class="nt-branch-group"><div class="nt-branch-group-head">Local</div>`;
      for (const b of app._branchData.local) { const isCurrent = b === app._branchData.current; html += `<div class="nt-branch-item" data-type="local" data-value="${esc(b)}" onclick="selectBranch(this)"><span>${esc(b)}</span>${isCurrent ? '<span class="nb-current">current</span>' : ''}</div>`; }
      html += '</div>';
    }
    if (app._branchData.remote?.length) {
      html += `<div class="nt-branch-group"><div class="nt-branch-group-head">Remote</div>`;
      for (const b of app._branchData.remote) { html += `<div class="nt-branch-item" data-type="remote" data-value="${esc(b)}" onclick="selectBranch(this)"><span style="color:var(--text-2)">${esc(b)}</span></div>`; }
      html += '</div>';
    }
