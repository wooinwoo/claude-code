...
addRoute('POST', '/api/projects/:id/auto-commit/plan', async (req, res) => {
  ...
  try {
    ...
    const truncatedDiff = allDiff.length > 20000 ? allDiff.slice(0, 20000) + '\n...(truncated)' : allDiff;
...
- Prefer fewer commits with clear logical grouping over many tiny commits`;
...
addRoute('POST', '/api/settings/import', async (req, res) => {
  try {
    ...
    let added = 0;
    const existingPaths = new Set(getProjects().map(p => p.path));
    for (const p of imported) {
      if (!p.name || !p.path) continue;
      if (existingPaths.has(p.path)) continue;
      addProject({ name: p.name, path: p.path, stack: p.stack, devCommand: p.devCommand, githubSlug: p.githubSlug, color: p.color });
...
addRoute('GET', '/api/discover-projects', (_req, res) => {
  const existingPaths = getProjects().map(p => p.path);
...
addRoute('POST', '/api/discover-projects/add', async (req, res) => {
  ...
  for (const p of paths) {
    ...
    if (existingPaths.has(normalPath)) continue;
    const project = addProject({ name: p.name, path: normalPath });
    registerProjectPollers(project);
    existingPaths.add(normalPath);
    added++;
  }
...
function registerProjectPollers(project) {
  poller.register(`session:${project.id}`, () => detectSessionState(project), POLL_INTERVALS.sessionStatus, 'session:status');
...
addRoute('POST', '/api/projects/:id/dev-server/start', async (req, res) => {
  ...
  const portRe = /(?:localhost|127\.0\.0\.1|0\.0\.0\.0|port\s*):?\s*(\d{4,5})/i;
  const scanPort = (data) => {
    if (ds.port) return;
    const m = data.toString().match(portRe);
    if (m) { ds.port = parseInt(m[1], 10); broadcastDevStatus(); }
  };
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      case 'create': {
        ...
        const shell = process.platform === 'win32' ? 'powershell.exe' : 'bash';
        const cwd = (msg.cwd || project.path).replace(/\//g, '\\');
